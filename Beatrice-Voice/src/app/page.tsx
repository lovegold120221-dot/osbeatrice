...
import { tools, executeTool, setEmbeddedAgentId, setEmbeddedOwnerUid, setTaskExecutorPermission, type TaskExecutorPermission } from "../services/tools";
import { skills, skillCategories, getSkillsByCategory, type SkillDefinition, type SkillCategory } from "../services/skills";
...
export default function App() {
  ...
  const [activeModal, setActiveModal] = useState<'account' | 'settings' | 'data' | 'skills' | null>(null);
  ...
  const [pairedAgentId, setPairedAgentId] = useState<string | null>(null);
  const [pairedOwnerUid, setPairedOwnerUid] = useState<string | null>(null);
  ...
  const firebaseOwnerUid = pairedOwnerUid ?? user?.uid ?? null;
  ...
  // Acknowledged Flutter <-> Web handshake. Flutter waits for a request,
  // responds with identity, and retries until the Web sends an ack.
  useEffect(() => {
    const onHandshakeResponse = (event: Event) => {
      const detail = (event as CustomEvent<{ type?: string; agentId?: string; ownerUid?: string; webSessionUid?: string }>).detail;
      if (detail?.type !== 'device.handshake.response') return;
      const agentId = detail?.agentId;
      const ownerUid = detail?.ownerUid;
      if (!agentId || !ownerUid) return;
      if (detail.webSessionUid && user && detail.webSessionUid !== user.uid) return;
      localStorage.setItem('eburon_pairedAgentId', agentId);
      localStorage.setItem('eburon_pairedOwnerUid', ownerUid);
      setEmbeddedAgentId(agentId);
      setEmbeddedOwnerUid(ownerUid);
      setPairedAgentId(agentId);
      setPairedOwnerUid(ownerUid);
      setAgentReady(true);
      console.log('[BRIDGE] device.handshake.response', { agentId, ownerUid });
      window.BeatriceBridge?.postMessage(JSON.stringify({
        type: 'device.handshake.ack',
        agentId,
        ownerUid,
        webSessionUid: user?.uid,
      }));
      // After ack, bind the web session to the agent/owner in Firebase.
      window.BeatriceBridge?.postMessage(JSON.stringify({
    ...
    window.addEventListener('beatrice-agent-event', onHandshakeResponse);
    return () => window.removeEventListener('beatrice-agent-event', onHandshakeResponse);
  }, [user]);

  // Periodically request a handshake until acknowledged.
  useEffect(() => {
    if (!user || !window.BeatriceBridge) return;
    const webSessionUid = user.uid;
    const storedAgentId = localStorage.getItem('eburon_pairedAgentId');
    const storedOwnerUid = localStorage.getItem('eburon_pairedOwnerUid');
    let attempts = 0;
    const maxAttempts = 15;
    const interval = 2000;
    let timeoutId: NodeJS.Timeout;
    const sendRequest = () => {
      if (pairedAgentId && pairedOwnerUid) {
        clearInterval(timeoutId);
        return;
      }
      if (attempts >= maxAttempts) {
        clearInterval(timeoutId);
        console.warn('[BRIDGE] handshake.request timeout');
        return;
      }
      attempts++;
      console.log('[BRIDGE] device.handshake.request', { attempt: attempts, webSessionUid, hintAgentId: storedAgentId });
      window.BeatriceBridge?.postMessage(JSON.stringify({
        type: 'device.handshake.request',
        webSessionUid,
        hintAgentId: storedAgentId,
        hintOwnerUid: storedOwnerUid,
      }));
    };
    sendRequest();
    timeoutId = setInterval(sendRequest, interval);
    return () => clearInterval(timeoutId);
  }, [user, pairedAgentId, pairedOwnerUid]);

  useEffect(() => {
    const onIdentity = (event: Event) => {
      const detail = (event as CustomEvent<{ agentId?: string; ownerUid?: string; state?: string; access?: string }>).detail;
      if (detail?.agentId) {
        setPairedAgentId(detail.agentId);
        setEmbeddedAgentId(detail.agentId);
      }
      if (detail?.ownerUid) {
        setPairedOwnerUid(detail.ownerUid);
    ...
    window.addEventListener('beatrice-agent-event', onIdentity);
    window.addEventListener('beatrice-task-event', onIdentity);
    return () => {
      window.removeEventListener('beatrice-agent-event', onIdentity);
      window.removeEventListener('beatrice-task-event', onIdentity);
    };
  }, []);
  ...
  // Device task status subscriptions are started from handleLiveToolCalls
  // *after* the tool response is sent, preventing a race with the outstanding
  // function response.

  const subscribeToTaskStatus = (taskId: string) => {
    if (!pairedAgentId) return;
    stopTaskListenerRef.current?.();
    const taskRef = dbRef(database, `deviceTasks/${pairedAgentId}/${taskId}`);
    let lastUpdate = '';
    stopTaskListenerRef.current = onValue(taskRef, (snapshot) => {
      const task = snapshot.val();
      if (!task) return;
      const update = `${task.status}:${task.statusMessage || task.summary || task.error || ''}`;
      if (update === lastUpdate) return;
      lastUpdate = update;
      console.log('[TASK] status.update', { taskId, status: task.status });
  ...
  const handleLiveClose = (event: { code?: number; reason?: string; wasClean?: boolean }, isVoice: boolean) => {
    ...
    const cleanCodes = new Set([1000, 1001]);
    const wasClean = event?.wasClean || cleanCodes.has(event?.code ?? -1);
    if (!wasClean && reconnectAttemptsRef.current < 1 && shouldReconnectRef.current && !isReconnectingRef.current) {
      reconnectAttemptsRef.current += 1;
      isReconnectingRef.current = true;
      console.log('[VOICE] live.reconnect attempt', reconnectAttemptsRef.current);
      setVoiceStatus('Reconnecting voice session...');
      setTimeout(() => {
        if (shouldReconnectRef.current) {
          if (isVoice) startLiveSession();
          else startChatLiveSession();
        }
  ...
  const handleLiveToolCalls = async (message: any) => {
    const calls = message.toolCall?.functionCalls;
    if (!calls?.length || !liveSessionRef.current) return;

    try {
      const callResults = await Promise.all(calls.map(async (call: any) => {
        const output = await executeTool(call.name, call.args || {});
        return { call, output };
      }));

      const functionResponses = callResults.map(({ call, output }) => ({
        id: call.id,
        name: call.name,
        response: { output },
      }));

      console.log('[VOICE] tool.response', functionResponses.map(r => ({ id: r.id, name: r.name, status: r.response.output?.status })));
      liveSessionRef.current?.sendToolResponse({ functionResponses });

      // AFTER the tool response is sent, subscribe to accepted device tasks.
      // This prevents a conversational turn from being injected while Gemini is
      // still waiting for the outstanding function response.
      for (const { call, output } of callResults) {
        if (call.name === 'executeTask' && output?.status === 'accepted' && output?.taskId) {
          subscribeToTaskStatus(output.taskId);
        }
      }
    } catch (error) {
      console.error('Live tool execution failed:', error);
    }
  };
  ...
  const startChatLiveSession = async () => {
    ...
    try {
      ...
      const sessionPromise = connectLive(
        ...
        (event) => handleLiveClose(event, false),
        sessionUserContext,
        responseStyle,
  ...
  const startLiveSession = async () => {
    try {
      ...
      const sessionPromise = connectLive(
        ...
        (event) => handleLiveClose(event, true),
        userContext,
        responseStyle,
        taskExecutorPermission,
      );
            ...
            <>
              ...
              >
                ...
                <div className="p-4 border-t border-neutral-800 space-y-2">
                  ...
                  <button onClick={() => setActiveModal('skills')} className={`w-full flex items-center space-x-3 p-3 rounded-xl transition-colors text-sm ${activeModal === 'skills' ? 'bg-white text-black' : 'hover:bg-[#212121] text-neutral-300'}`}>
                    <Wand2 size={18} />
                    <span>Skills</span>
                  </button>
                  {user && (
              ...
              >
                <div className="p-4 border-b border-neutral-800 flex justify-between items-center">
                  <h3 className="font-semibold text-white">
                    ...
                    {activeModal === 'skills' && 'Available Skills'}
                  </h3>
